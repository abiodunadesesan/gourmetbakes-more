...
import { Loader2, SlidersHorizontal, PackageX, ChevronDown } from "lucide-react";
...
function MenuContent() {
    ...
    const fetchProducts = useCallback(async (isLoadMore = false) => {
        const page = isLoadMore ? meta.page + 1 : 1;
    ...
    const handleCategoryToggle = (category: string) => {
        const newCats = categories.includes(category)
                ...
                <div className="flex flex-col lg:flex-row gap-10">
                    ...
                    <div className="hidden lg:block w-72 shrink-0">
                        <FilterSidebar
                            ...
                            onPriceChange={(range) => updateUrl({ min_price: range[0].toString(), max_price: range[1].toString() })}
                            inStockOnly={inStock}
                            onInStockToggle={() => updateUrl({ in_stock: (!inStock).toString() })}
                    ...
                    <div className="flex-1 space-y-8">
                        ...
                        <FilterChips
                            q={q}
                            onClearQ={() => updateUrl({ q: null })}
                            categories={categories}
                            onRemoveCategory={handleCategoryToggle}
                            priceRange={[minPrice, maxPrice]}
                            onResetPrice={() => updateUrl({ min_price: null, max_price: null })}
                            inStockOnly={inStock}
                            onToggleInStock={() => updateUrl({ in_stock: null })}
                            onClearAll={() => router.push(pathname)}
                        />
